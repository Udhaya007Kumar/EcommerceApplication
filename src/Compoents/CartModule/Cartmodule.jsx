...
const Cartmodule = ({isOpen,openNav,closeNav,cartQuality,CardRomvebutton,cartTotalValue,setCartTotalValue}) => {
    const [cardAddValue, setcardAddValue] =useState(1)

   const CardADDAmountValue =()=>{
    setcardAddValue((prevalue)=>prevalue + 1)
    setCartTotalValue((preTotalvalue)=>preTotalvalue * 2)

   }

   const CardDeAmountValue =()=>{
    if(cardAddValue !== 0){
        setcardAddValue((prevalue)=>prevalue - 1)
    ...
    return (
       ...
       <div className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <h5 className=' text-center lg:text-start xl:text-start m-5 text-3xl text-white font-bold'>MyCart</h5>
         {
            cartQuality.map((ele,index)=>{
                return(
                    <div className='bg-slate-300 m-5 xl:bg-gray-50  xl:m-5 rounded-lg'>
                        <div class=' flex xl:flex justify-stretch '>
                            <div className='  xl:m-2 rounded-2xl  '>
                               <img className='object- h-48 w-[12rem]' width={50} src={ele.image}></img>
                            </div>
                            ...
                            <div className='mt-8  ml-12 w-1/4 gap-2 '>
                                {/* <button className='bg-yellow-400 px-4 py-3 rounded-2xl hover:bg-yellow-700 hover:text-white'>ADD</button> */}
        ...
        <div className=' flex  justify-end mr-12 xl:flex xl:justify-between '>
        <h5  className=' text-center lg:text-start xl:text-start m-5 text-3xl text-white font-bold'>Total</h5>
        <h5 className=' text-end  lg:text-start xl:text-start m-5 text-3xl text-white font-bold'>{cartTotalValue}</h5>
        </div>
